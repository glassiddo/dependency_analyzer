...
countries <- c( 
  "BEN", "BWA", "BFA", "CMR", "GHA", "CIV", "GIN","KEN", "LSO", 
  "MLI", "MUS", "MOZ", "SEN", "SLE", "ZAF", "TZA", "UGA", "ZMB"
)

ctries_to_add_aggs <- c("GAB")

# yr_start/end_dic only feature countries with the MIGYRS1 variable + ZAF (?)
yr_start_dic <- c("BEN"=1992, "CMR"= 1977, "GIN"= 1986, 
                  "KEN"=1989, "MLI"=1988, "ZAF"=1996, "UGA"=1992)
yr_end_dic <- c("BEN"=2001, "CMR"=1986, "GIN"=1995, 
                "KEN"=1998, "MLI"=1997, "ZAF"=2000, "UGA"=2001)

# for the others - number of years for migration variable
period_dic <- c("BWA"= 5, "BFA"=NA, "GHA"=5, "LSO"=10, "CIV"=1, "MUS"=5,
                "MOZ"=NA, "SEN"=5, "SLE"=NA, "TZA"=NA, "ZMB"=NA)
...
#imputing m_od, m_oo, etc for internal migrants from census countries ----
dat_df <- data.frame()
...
for (ctry_str in countries){
  ...
  census_data <- census_info %>% 
    ...
    select(yr1_suffix, lvl, prime_age)
  
  census_yr <- census_data$yr1_suffix
  lvl <- census_data$lvl
  ...
  if (prime_age == TRUE) 
  { ctry_df %<>% filter(age >= min_pa & age <= max_pa) }
  ...
  print(sum(ctry_df$d_shares))
  dat_df <- bind_rows(dat_df, ctry_df) 
}

## add countries from aggregates (non census, so pdfs/lsms)
for (ctry in ctries_to_add_aggs) {
  yr <- census_info %>% filter(country == ctry) %>% pull(yr1_suffix)
  agg_df <- readRDS(here(
...
mig_df <- dat_df %>%
  relocate(country_name, prev_country_name, 
           ipums_id_d, ipums_id_o, district, prev_district, 
...
# create a lookup table that maps every [country, prev_country, district/region]
# combination to its corresponding pcd_shares value, 
# where both district and region values are included as potential match locations
...
saveRDS(final_df, here(out.dir, "R", "full_migration_census1.rds"))
write_dta(final_df, here(out.dir, "full_migration_census1.dta"))
...
emig_df <- final_df %>%
  ...
  rename(bl_emig = m_o, bl_iemig = intm_o, ipums_id = ipums_id_o) %>%
  rename_with(~paste0("bl_e", .), starts_with("mig")) %>%
  mutate(bl_emig_ann = bl_emig / bl_emig_prd_len,
         bl_log_emig = ifelse(bl_emig > 0, log(bl_emig), 0),
         bl_log_emig_ann = bl_log_emig / bl_emig_prd_len,
         bl_iemig_ann = bl_iemig / bl_emig_prd_len,
         bl_ilog_emig = ifelse(bl_iemig > 0, log(bl_iemig), 0),
         bl_ilog_emig_ann = bl_ilog_emig / bl_emig_prd_len,
         bl_emig_measure = paste0("window: ", bl_emig_prd_len, " year")) %>%
  left_join(id_df, by = "ipums_id") %>%
  relocate(country_name, ipums_id, admin_name, geo_lvl, 
           bl_emig_start_yr, bl_emig_end_yr, bl_emig_prd_len, bl_emig_measure) %>%
  var_labels(
    ...
    bl_iemig = "baseline international emigration, prime-aged",
    bl_iemig_ann = "baseline annual international emigration, prime-aged",
    bl_ilog_emig = "baseline log international emigration, prime-aged",
    bl_ilog_emig_ann = "baseline annual log international emigration, prime-aged",
...
saveRDS(emig_df, here(out.dir, "R", "emigration_census1.rds"))
write_dta(emig_df, here(out.dir, "emigration_census1.dta"))

immig_df <- final_df %>%
  filter(country_name == prev_country_name) %>%
  select(ipums_id_d, ends_with("m_d"), starts_with("mig")) %>%
  distinct(ipums_id_d, .keep_all = TRUE) %>%
  rename(bl_immig = m_d, bl_iimmig = intm_d, ipums_id = ipums_id_d) %>%
  rename_with(~paste0("bl_im", .), starts_with("mig")) %>%
  mutate(bl_immig_ann = bl_immig / bl_immig_prd_len,
         bl_log_immig = ifelse(bl_immig > 0, log(bl_immig), 0),
         bl_log_immig_ann = bl_log_immig / bl_immig_prd_len,
         bl_iimmig_ann = bl_iimmig / bl_immig_prd_len,
         bl_ilog_immig = ifelse(bl_iimmig > 0, log(bl_iimmig), 0),
         bl_ilog_immig_ann = bl_ilog_immig / bl_immig_prd_len,
         bl_immig_measure = paste0("window: ", bl_immig_prd_len, " year")) %>%
  left_join(id_df, by = "ipums_id") %>%
  relocate(country_name, ipums_id, admin_name, geo_lvl, 
           bl_immig_start_yr, bl_immig_end_yr, bl_immig_prd_len, bl_immig_measure) %>%
  var_labels(
    ...
    bl_iimmig = "baseline international immigration, prime-aged",
    bl_iimmig_ann = "baseline annual international immigration, prime-aged",
    bl_ilog_immig = "baseline log international immigration, prime-aged",
    bl_ilog_immig_ann = "baseline annual log international immigration, prime-aged",
...
saveRDS(immig_df, here(out.dir, "R", "immigration_census1.rds"))
write_dta(immig_df, here(out.dir, "immigration_census1.dta"))
