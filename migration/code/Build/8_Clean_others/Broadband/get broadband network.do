...
forval i=1/2 {
	...
	*/
	...
	cd "${dropbox}\Migration Africa\data\Build\Africa\Broadband\"
		...
		*shp2dta using electricity_`y'_ipums`i'.shp, data(electricity_`y'_ipums`i'_data) coord(electricity_`y'_ipums`i'_coords) replace
		spshape2dta broadband_ipums`i'.shp,  replace
		
		
		use ipums_id _ID using broadband_ipums`i'.dta, clear
		
		gen has_bb=1
		
		gcollapse (max) has_bb, by(ipums_id)
