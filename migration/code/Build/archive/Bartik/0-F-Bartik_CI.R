...
f_pop <- function(start_df, end_df, lvl = 2) {
  ...
  pop_d <- pop %>%
    mutate(delta_Nd = N1_pa - N0_pa,
           ...
           delta_pop = (N1_all-N0_all)/N0_all/period_len) %>%
    select({{geo}}, starts_with("delta_"), starts_with("log_gr"), starts_with("N1"), starts_with("N0"))%>%
    rename_with(~paste("D_", .x, sep = ""),starts_with("delta_")| starts_with("log_gr")| starts_with("N1")|starts_with("N0"))
  
# origin
  pop_o <- pop_d %>%
    rename({{prev_geo}} := {{geo}})%>%rename_with(~gsub("D_","O_", .x, fixed = TRUE),starts_with("D_") )
...
f_orig_pop <- function(dat_df, period_len, lvl = 2) {
  # create the following (+ naming convention)
  # 1. delta log pop at o using full population (log(N_o1_all/N_o0_all))
  # 2. delta N_dt using prime aged (delta_Nd = N_d1_pa - N_d0_pa)
  # 3. Origin time 0 prime aged pop for Bartik
  ...
  pop_df <- dat_df %>% 
    ...
    summarise(
      # in case someone has missing weight
      O_all = round(sum(wgt, na.rm = TRUE)),
      # keep people with no age reported, but not as prime aged
      O_pa = round(sum(prime_age, na.rm = TRUE)),
      # agricultural employment weight
      O_ag = sum(wgt * empl_ag, na.rm = TRUE),
      O_nonag = sum(wgt * employed * (1 - empl_ag), na.rm = TRUE),
      O_mining = sum(wgt * empl_mining, na.rm = TRUE),
      O_nonmining = sum(wgt * employed * (1 - empl_mining), na.rm = TRUE),
      O_empl = sum(wgt * employed, na.rm = TRUE),
      O_u = sum(wgt * (1 - employed), na.rm = TRUE),
      O_nam = sum(wgt * empl_nam, na.rm = TRUE),
      O_rural = sum(wgt * (1 - urban), na.rm = TRUE),
      O_urban = sum(wgt * urban, na.rm = TRUE),
  ...
  pop_df <- full_join(distset_df, pop_df) %>%
    drop_na(distset) %>%
    select(-distset) %>%
    mutate(period_len = period_len)
}
...
f_nitelite <- function(dat_df = nl_df, yr_start, yr_end, lvl = 2) {
  ...
  yr_start <- max(yr_start, 1995)
  yr_end <- min(yr_end, 2013)
  
  geo_name <- sym(paste0(g_sym(lvl), "_name"))

  dat_df=dat_df%>%group_by({{geo_name}})%>%mutate(across(ntla:ntlv,~rollmean(.x,5,align="right",fill=NA)))
  
  # night light in first observed year (1992)
  nl_t0 <- dat_df %>% filter(year == 1995) %>%
  ...
  delta_nl <- full_join(nl_start, nl_end) %>%
    ...
    mutate(
      ...
      # lagged changes
      delta_lag_ntla = (bl_ntla - t0_ntla) / (yr_start - 1995),
      delta_log_lag_ntlv = (bl_log_ntlv - t0_log_ntlv) / (yr_start - 1995)) %>%
...
f_croparea <- function(dat_df, yr_start, yr_end, lvl = 2) {
  ...
  delta_crop_df <- dat_df %>%
    ...
    mutate(
      ...
      # annualized
      delta_log_sharecrop = (log_share - lag(log_share))/(yr_end - yr_start),
      delta_sharecrop = (sharecrop - lag(sharecrop)),
      delta_sharecrop_ann = (sharecrop - lag(sharecrop))/total_area/(yr_end - yr_start),
      sharecrop_bl = lag(sharecrop),
      sharecrop_el = sharecrop,
      total_area_cropdat = total_area) %>%
    filter(year == yr_end) %>%
    select({{geo_name}}, delta_log_sharecrop, delta_sharecrop, sharecrop_bl, 
           sharecrop_el, delta_sharecrop_ann, total_area_cropdat)
...
f_bartik <- function(ctry3, mig_df = mig_od, dlta_pop = pop_df, 
  ...
  Btk_df <- full_join(dlta_pop, mig_df) %>%
    ...
    mutate(
      ...
      Bm_d    = d_shares * (D_delta_Nd_avg / O_N0_pa),
      Bm_o    = o_shares * (D_delta_Nd_avg / O_N0_pa),
      Bd      = inv_dist_shares * (D_delta_Nd_avg / O_N0_pa),
      Bd_avg    = avg_dist_shares * (D_delta_Nd_avg / O_N0_pa),
      # Bartiks for delta log non agricultural employment
      Bm_d_gr_nonag = d_shares * (D_log_gr_nonag),
      Bm_o_gr_nonag = o_shares * (D_log_gr_nonag),
      Bd_gr_nonag   = inv_dist_shares * (D_log_gr_nonag),
      Bd_avg_gr_nonag = avg_dist_shares * (D_log_gr_nonag),
      # Bartiks for delta log pop at destination
      Bm_d_dlp = d_shares * (D_delta_log_pop_d_avg),
      Bm_o_dlp = o_shares * (D_delta_log_pop_d_avg),
      Bd_dlp   = inv_dist_shares * (D_delta_log_pop_d_avg),
      Bd_avg_dlp = avg_dist_shares * (D_delta_log_pop_d_avg),
      # other miscellaneous variants
      Bm_d_dom    = (m_od_dom / m_d_dom)* (D_delta_Nd_avg / O_N0_pa),
      Bm_d_nonag  = d_shares * (D_delta_Nd_nonag / O_N0_pa),
      Bd_nonag    = inv_dist_shares * (D_delta_Nd_nonag / O_N0_pa),
      Bd_avg_nonag    = avg_dist_shares * (D_delta_Nd_nonag / O_N0_pa),
      Bm_d_urban  = d_shares * (D_delta_Nd_urban / O_N0_pa),
      Bm_d_ag     = d_shares * (D_delta_Nd_ag / O_N0_pa),
      Bd_urban    = inv_dist_shares * (D_delta_Nd_urban / O_N0_pa),
      Bd_ag       = inv_dist_shares * (D_delta_Nd_ag / O_N0_pa),
      Bm_d_mining = d_shares * (D_delta_Nd_mining / O_N0_pa),
      Bd_mining    = inv_dist_shares * (D_delta_Nd_mining / O_N0_pa),
  ...
  B_i_df <- mig_df %>%
    ...
    full_join(dlta_pop) %>%
    mutate(
      Bi_all = (m_do_all / m_d_all)* (D_delta_Nd_avg / O_N0_pa),
      Bi_dom = (m_do_dom / m_d_dom)* (D_delta_Nd_avg / O_N0_pa),
      Bi_ag = (m_do_all / m_d_all)* (D_delta_Nd_ag / O_N0_pa),
      Bi_nonag = (m_do_all / m_d_all)* (D_delta_Nd_nonag / O_N0_pa),
      Bi_urban = (m_do_all / m_d_all)* (D_delta_Nd_urban / O_N0_pa)) %>%
    filter({{geo}} != {{prev_geo}}) %>%
    group_by({{prev_geo}}) %>%
...
#    mutate(delta_log_age = log_age_ypl - log_age_opl)
  ...
  #   filter({{geo}} == {{prev_geo}}) %>%
  #   mutate(bl_emig_rate = 100 * bl_emig / O_N0_pa ) %>%
  #   select({{geo}}, bl_emig_rate, bl_emig)
  # 
...
#             ~log(1 + .x), .names = "log_{.col}"),
      ...
      O_rural_pop = O_N0_rural / O_N0_all,
      O_ag_pop = O_N0_ag / O_N0_all,
...
f_bartik_nopop <- function(ctry3, mig_df = mig_od, orig_pop = pop_df,
                     fl_df = loss_df, dist_df = distance_df, 
                     urban_df = urb_df, 
                     nitelite_df = delta_nl, crop_df = delta_crop, lvl = 2) {
  # vars I need to create
  ...
  distset <- svy_districts(orig_pop, l = lvl) %>%
    select(-starts_with("prev")) %>%
    # select(-distset) %>%
    unique()
...
add_labs <- function(dat_df) {
  out_df <- dat_df %>%
    var_labels(O_N0_ag = "Baseline ag workers",
               O_N0_rural = "Baseline rural population",
               O_N0_all = "Baseline rural population",
               el_emig_rate = "endline emigration rate",
               O_delta_Nd = "change in prime age pop",
               O_delta_Nd_avg = "change in prime age pop per year",
               O_delta_Nd_ag = "change in ag empl pop per year",
               O_delta_Nd_nonag = "change in nonag empl pop per year",
               O_delta_Nd_mining = "change in mining employment per year",
               O_delta_Nd_nonmining = "change in nonmining employment per year",
               O_delta_Nd_nam = "change in non-ag, non-mining employment per year",
               O_delta_Nd_empl = "change in employment per year",
               O_delta_Nd_rural = "change in rural pop per year",
               O_delta_Nd_urban = "change in urban pop per year",
               O_log_gr = "log period 2 total pop / period 1 total pop",
               O_log_gr_ann = "log gr annualized rate",
               O_log_gr_ag = "annualized growth rate of ag employment",
               O_log_gr_ag = "annualized growth rate of non-ag employment",
               O_log_gr_mining = "annualized growth rate of mining employment",
               O_log_gr_nam = "annualized growth rate of non-ag, non-mining employment",
               O_log_gr_empl = "annualized growth rate of employment",
               O_log_gr_rural = "annualized growth rate of rural pop",
               e_o = "average annual emigration",
               log_emig = "log 1 + average annual emigration",
               log_emig = "log emig",
               el_emig_rate = "annual emigration rate",            
               ...
               #log_Bm_d_urban = "$\\log\\text{B}_{m}^{\\text{urb}}$",
               #log_Bi_urban = "$\\log\\text{B}_{i}^{\\text{urb}}$",
               #log_Bd_urban = "$\\log\\text{B}_{d}^{\\text{urb}}$",
               ...
               deforestation="total forest loss / total administrative area",
               #delta_log_sharecrop = "Delta log crop area share",
               delta_sharecrop = "Delta croparea share",
               sharecrop_bl = "baseline croparea",
               sharecrop_el = "endline croparea")
}
