...
f_pop <- function(start_df, end_df, lvl = 2) {
  ...
  pop_o <- pop %>%
    mutate(log_gr = log(N1_all / N0_all),
           ...
           N_o0_u = N0_u,
           N_o0_rural = N0_rural) %>%
    select({{geo}}, ends_with("wgt"), starts_with("delta_"), starts_with("log_gr"),
...
f_nitelite <- function(dat_df = nl_df, yr_start, yr_end, lvl = 2) {
  ...
  dat_df=dat_df%>%group_by({{geo_name}})%>%mutate(across(ntla:ntlv,~rollmean(.x,5,align="center",fill=NA)))
...
f_croparea <- function(dat_df, yr_start, yr_end, lvl = 2) {
  ...
  delta_crop_df <- dat_df %>%
    ...
    mutate(
      ...
      # annualized
      delta_log_share = (log_share - lag(log_share))/(yr_end - yr_start),
    ...
    filter(year == yr_end) %>%
    select({{geo_name}}, delta_log_share, delta_sharecrop, sharecrop_bl, 
...
f_bartik <- function(ctry3, mig_df = mig_od, dlta_pop = pop_df, 
  ...
  Btk_df <- full_join(dlta_pop, mig_df) %>%
    ...
    mutate(
      ...
      Bd_avg    = avg_dist_shares * (delta_Nd_avg / N_o0),
      # Bartiks for delta log non agricultural employment
      Bm_d_gr_nonag = d_shares * (log_gr_nonag),
      Bm_o_gr_nonag = o_shares * (log_gr_nonag),
      Bd_gr_nonag   = inv_dist_shares * (log_gr_nonag),
      Bd_avg_gr_nonag = avg_dist_shares * (log_gr_nonag),
      ...
      Bd_avg_dlp = avg_dist_shares * (delta_log_pop_d_avg),
      ...
      Bd_avg_nonag    = avg_dist_shares * (delta_Nd_nonag / N_o0),
      Bm_d_urban  = d_shares * (delta_Nd_urban / N_o0),
      Bm_d_ag     = d_shares * (delta_Nd_ag / N_o0),
      Bd_urban    = inv_dist_shares * (delta_Nd_urban / N_o0),
      Bd_ag       = inv_dist_shares * (delta_Nd_ag / N_o0),
      Bm_d_mining = d_shares * (delta_Nd_mining / N_o0),
      Bd_mining    = inv_dist_shares * (delta_Nd_mining / N_o0),
  ...
  B_i_df <- mig_df %>%
    ...
    mutate(
      ...
      Bi_urban = (m_do_all / m_d_all)* (delta_Nd_urban / N_o0)) %>%
    filter({{geo}} != {{prev_geo}}) %>%
    group_by({{prev_geo}}) %>%
