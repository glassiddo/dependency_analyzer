...
save_ctry <- function(dat_df, ctry_str, lvl = 2) {
  fl_str <- paste0(build.dir, ctry_str, "/Migration/l", lvl,"_Bartik_BHJ")
...
read_ctry <- function( ctry_str, lvl = 2) {
  out_df <- readRDS(
    paste0(build.dir, ctry_str, "/Migration/l", lvl, "_Bartik_BHJ.rds")
    )
}
...
geo_forestloss <- function( dat_df, lvl, yr_start, yr_end) {
  ...
  # convert from sq-hectares to km^2
  dat_df %<>% mutate(across(c(treecover, loss), ~.x/ 10^6) )
...
#*******************************************************************************
##### f_bhj(mig_df, pop_df, loss_df, dist_df, ctry3)
#*******************************************************************************
#* birth_df = birth_od, birth_dlta_pop = pop_df, birth_lvl = "district", 
f_bhj <- function(ctry3, mig_df = mig_od, dlta_pop = pop_df, 
  ...
  o_df<-full_join(fl_df,crop_df)
  o_df%>%mutate(deforestation=avg_loss/total_area)
  # step 1 create the bartik instruments
  bhj_df <- full_join(dlta_pop, mig_df) %>%
    full_join(dist_df) %>% full_join(fl_df) %>% full_join(crop_df) %>%
    ...
    mutate(
      ...
      bhj_m_delta_log_share    = d_shares * delta_log_share ,
      bhj_m_deforestation    = d_shares * deforestation,
      bhj_d_delta_log_share     = inv_dist_shares * delta_log_share ,
      bhj_d_deforestation    = inv_dist_shares * deforestation,
  ...
  out_df <- full_join(distset, orig_pop) %>%
    ...
    mutate(
      ...
      deforestation=total_loss / total_area,
