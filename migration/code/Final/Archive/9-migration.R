...
od_flow <- function(ctry_str, lvl = 2, cens_yr, yr_start, yr_end, 
  ...
  #selects either (prev) district or region depending on the desired level
  geo <- g_sym(lvl)
  ...
  sample_df <- dat_df %>%
    ...
    summarise(m_od = sum(migrant, na.rm = TRUE),
              m_oo = sum(m_oo, na.rm = TRUE),  #nb here m_oo can also be >0 if o≠d, if the person did not migrate within the desired period
              .groups = 'drop') %>%
    ...
    mutate(m_d = sum(m_od),
           m_oo = sum(m_oo)) %>%
    #* sum m_oo bc it counts all the people found at destination d, who are not 
    #* considered as migrants bc they moved from their previous destination at 
    #* another period/year than the one considered
    ungroup() %>%
    group_by({{prev_geo}}) %>%
